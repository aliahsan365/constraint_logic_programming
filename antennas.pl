...
% Use the SAT variable : covering-C-S
maxStations(8).
numStations(20).
city(1,[12,8,1,5,6,9]).
city(2,[6,14,17,10,2]).
...
stations(S):- numStations(N), between(1,N,S).
...
noCoveringOrUsed:- city(C,L),stations(S),member(S,L),writeClause([\+covering-C-S,used-S]),fail.
noCoveringOrUsed.
%% Display solution
